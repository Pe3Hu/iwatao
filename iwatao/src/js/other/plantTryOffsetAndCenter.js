...
class plant {
  constructor ( index, center, orientation, onSreen ){
    this.index =  index;
    this.center = center;
    this.orientation = orientation;
    this.onSreen = onSreen;
    this.const = {
      ring: 10,
      parts: 3,
      a: cellSize,
  ...
  initPrimaryDelta(){
    this.array.delta[0].push( new delta( this.var.index, this.orientation, createVector(), this.center ));
  ...
  initTrueDelta( i, p ){
    ...
    for (let j = 0; j < this.var.far; j++){
      ...
      this.array.delta[i].push( new delta( this.var.index, orientation, vec1.copy(), this.center.copy() ));
      this.var.index++;
    }
  ...
  addDelta( index ){
    //check borders array of option
    /*if( typeof this.array.option[index] === 'undefined' )
       return;*/
